...
namespace Smooth
{
    ...
    /// </remarks>
    public class SmoothSyncPUN : PunBehaviour, IPunObservable
    {
        ...
        /// </remarks>
        public delegate bool validateStateDelegate(StatePUN receivedState, StatePUN latestVerifiedState);
        ...
        /// </remarks>
        public static bool validateState(StatePUN latestReceivedState, StatePUN latestValidatedState)
        {
            return true;
        }
        ...
        [NonSerialized]
        public SmoothSyncPUN[] childObjectSmoothSyncs = new SmoothSyncPUN[0];
        ...
        NetworkStatePUN sendingTempState;
        ...
        public void Awake()
        {
            ...
            int calculatedStateBufferSize = ((int)(PhotonNetwork.sendRateOnSerialize * interpolationBackTime) + 1) * 2;
            stateBuffer = new StatePUN[Mathf.Max(calculatedStateBufferSize, 30)];
            ...
            if (childObjectToSync)
            {
                ...
                childObjectSmoothSyncs = GetComponents<SmoothSyncPUN>();
            ...
            targetTempState = new StatePUN();
            sendingTempState = new NetworkStatePUN();
        ...
        void interpolate(float interpolationTime)
        {
            ...
            StatePUN end = stateBuffer[Mathf.Max(stateIndex - 1, 0)];
            // The starting playback State.
            StatePUN start = stateBuffer[stateIndex];

            // Calculate how far between the two States we should be.
            float t = 1;
            if (end.ownerTimestamp != start.ownerTimestamp) // Avoid divide by 0
            {
                t = (interpolationTime - start.ownerTimestamp) / (end.ownerTimestamp - start.ownerTimestamp);
            }

            shouldTeleport(start, ref end, interpolationTime, ref t);

            // Interpolate between the States to get the target State.
            targetTempState = StatePUN.Lerp(targetTempState, start, end, t);
        ...
        bool extrapolate(float interpolationTime)
        {
            ...
            if (extrapolationMode != ExtrapolationMode.None && stateCount >= 2)
            {
                ...
                {
                    if (stateBuffer[0].ownerTimestamp != stateBuffer[1].ownerTimestamp)  // Avoid divide by 0
                    {
        ...
        void shouldTeleport(StatePUN start, ref StatePUN end, float interpolationTime, ref float t)
        {
        ...
        public void addState(StatePUN state)
        {
            if (stateCount > 1)
            {
        ...
        {
            photonView.RPC("RpcClearBuffer", PhotonTargets.All);
        }
        ...
        public void teleportOwnedObjectFromOwner()
        {
            ...
            photonView.RPC("RpcTeleport", PhotonTargets.Others, getPosition(), getRotation().eulerAngles, getScale(), localTime);
        }
        /// <summary>
        /// Teleport the object, the transform will not be interpolated on non-owners.
        /// </summary>
        ...
        public void teleportAnyObject(Vector3 newPosition, Quaternion newRotation, Vector3 newScale)
        {
            ...
            if (!photonView.isMine)
            {
                photonView.RPC("RpcNonServerOwnedTeleportFromServer", PhotonTargets.Others, newPosition, newRotation.eulerAngles, newScale);
            }
        }
        [PunRPC]
        ...
        {
            StatePUN teleportState = new StatePUN();
        ...
        /// </summary>
        void addTeleportState(StatePUN teleportState)
        {
        ...
        public void OnPhotonSerializeView(PhotonStream stream, PhotonMessageInfo info)
        {
            ...
            else
            {
                ...
                if (receivedState)
                {
                    NetworkStatePUN networkState = new NetworkStatePUN(this);
        ...
        void adjustOwnerTime()
        {
            ...
            if (receivedStatesCounter < PhotonNetwork.sendRateOnSerialize ||
            ...
            //    {
            //        StatePUN simulatedState = new StatePUN();
