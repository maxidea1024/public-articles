...
문서화는 `API` 정의시에 매우 중요한 부분입니다. 대부분의 경우 작성자와 사용자가 다르기 때문입니다. 통상적으로 이부분을 상쇄하기 위해서, 별도의 `API` 사용법을 담은 문서작업을 하게 됩니다. 이 작업은 적지 않은 부담이며, 개발자들에게는 때로는 고통일 수 있습니다. `PROM`에서는 `thrift` 처럼 `javadoc` 스타일의 주석을 달아주면 자동으로 문서화게 되도록 설계되어 있습니다. 이를 잘 활용하면, 별도의 `API` 문서작업이 생략될수도 있습니다. 또한, `PROM`은 자체적으로 `markdown` 태그를 사용해, `.html` 파일로 출력하는 기능을 지원합니다.
